...
<div style="max-width:620px">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:2rem;padding-bottom:1.25rem;border-bottom:1.5px solid #d0e2f7">
        ...
        <div>
            ...
            <div style="font-size:0.8rem;color:#8aaac8;font-weight:500">Update the details for <strong style="color:#4a6890">{{ $product->title }}</strong></div>
        </div>
    </div>
    ...
    <div style="margin-top:1.25rem;background:#fff;border:1px solid #fca5a5;border-radius:12px;padding:1rem 1.25rem;display:flex;align-items:center;justify-content:space-between;gap:12px">
        <div>
            ...
            <div style="font-size:0.8rem;color:#ef4444">Permanently delete this training and all its data.</div>
        </div>
        <form method="POST" action="/vendor/training/delete/{{ $product->id }}" onsubmit="return confirm('Delete this training? This cannot be undone.')">
            @csrf
            @method('DELETE')
            <button
                type="submit"
