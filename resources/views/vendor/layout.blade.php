...
.sidebar-nav{
  padding:1rem 0.75rem;flex:1;display:flex;flex-direction:column;gap:3px;
}
.nav-label{
...
.nav-link.danger:hover{background:var(--red-light);color:var(--red)}
.nav-icon{
...
.vendor-name{font-size:0.85rem;font-weight:600;color:var(--text-primary)}
.vendor-type{
...
.topbar-title{
  font-family:'Bebas Neue',sans-serif;
  font-size:1.4rem;letter-spacing:0.05em;color:var(--text-primary);
}
...
/* ── Content ── */
.content{flex:1;padding:2rem 1.75rem}
...
.panel-footer span{font-size:0.8rem;color:var(--text-muted)}
.panel-footer strong{color:var(--text-secondary)}
